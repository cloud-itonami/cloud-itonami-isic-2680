(ns magopticalmedia.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `magopticalmedia.store` ns docstring for why a second (Datomic-
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "coat-001"))))
    (is (true? (:registered? (store/equipment-unit s "coat-001"))))
    (is (false? (:verified? (store/equipment-unit s "stamp-002"))))
    (is (false? (:registered? (store/equipment-unit s "stamp-002"))))
    (is (= ["coat-001" "stamp-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest fresh-store-has-no-batches-or-equipment
  (let [s (store/mem-store)]
    ...
    (is (nil? (store/equipment-unit s "coat-001")))))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "coat-001" (:equipment-id (store/maintenance s "mnt-1"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "stamp-002" :severity :high}})
