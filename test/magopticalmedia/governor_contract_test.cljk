(ns magopticalmedia.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control coating/molding/stamping-line
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue a content-replication
  licensing/source-identification authorization mark') implemented
  faithfully. The single invariant under test:

    MagOpticalMediaAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Magnetic and Optical Media
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-coating-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the content-replication licensing/source-identification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-substrate-thickness-mm-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:substrate-thickness-mm 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-substrate-thickness-mm} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:substrate-thickness-mm (store/batch db "batch-001"))) "fabricated substrate-thickness-mm never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "coat-001" :severity :moderate
                                            :description "solvent vapor concentration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "coat-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
