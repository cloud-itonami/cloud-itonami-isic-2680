(ns magopticalmedia.operation-test
  "Smoke tests for the compiled MagOpticalMediaOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `magopticalmedia.governor-contract-test`; the Store contract in
  `magopticalmedia.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MagOpticalMediaOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "coat-001" :severity :moderate :description "solvent vapor concentration anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
