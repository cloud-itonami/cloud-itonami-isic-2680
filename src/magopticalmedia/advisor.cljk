(ns magopticalmedia.advisor
  "MagOpticalMediaAdvisor -- the *contained intelligence node* for the
  magnetic/optical media plant-operations coordination actor.

  It normalizes production-batch patches (product-type/substrate-
  thickness-mm/quantity/defect-rate), drafts a coating/molding/
  stamping-line-equipment maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound magnetic/optical-media product shipment coordination
  ...
  fields it cited), never a committed record and NEVER a real coating/
  molding/stamping-line-equipment actuation, freight dispatch, or
  content-replication-licensing authorization -- see README `What this
  actor does NOT do`. Every output is censored downstream by
  `magopticalmedia.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `magopticalmedia.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct coating/
  ...
  `magopticalmedia.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  substrate-thickness-mm, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a coating/molding/stamping-line-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `magopticalmedia.governor` NEVER trusts this report --
...
(defn- flag-safety-concern
  "Draft a solvent-coating chemical-hazard/equipment-safety concern.
  ...
  technicality). See `magopticalmedia.phase`: no phase ever adds this
  op to a phase's `:auto` set; `magopticalmedia.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound magnetic/optical-media product shipment
  ...
  invent one, and `magopticalmedia.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは磁気・光ディスクメディア(CD・DVD・Blu-ray・磁気テープ・"
       "磁気ストライプカード)製造工場"
       ...
       "コーティング・成形・スタンピング等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "コンテンツ複製ライセンス/ソース識別コード(IFPI SID等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `magopticalmedia.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :magopticalmedia-advisor-proposal
   :op         (:op request)
