(ns magopticalmedia.governor
  "Magnetic and Optical Media Plant Operations Governor -- the
  independent compliance layer that earns the MagOpticalMediaAdvisor
  ...
  draft-schedule) coating/molding/stamping-line equipment, whether a
  proposal secretly tries to self-issue a content-replication
  licensing/source-identification authorization mark (an authority
  ...
  becomes direct coating/molding/stamping-line-equipment control, so
  this MUST be a separate system able to *reject* a proposal and fall
  back to HOLD.

  `:itonami.blueprint/governor` is
  `:magnetic-optical-media-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:coating-line/
                                       actuate` or `:stamping-press/
                                       run`) is the 'direct coating/
                                       molding/stamping-line-equipment
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       through.
   11. Invalid substrate-thickness-
       mm                          -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:substrate-thickness-mm` that is
                                       not a physically plausible
                                       disc/tape/card thickness reading
                                       (`magopticalmedia.registry/
                                       substrate-thickness-mm-valid?`),
   ...
   12. Invalid defect-rate         -- for `:log-production-batch`, if
                                       ...
                                       not a physically plausible
                                       reading (`magopticalmedia.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [magopticalmedia.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct coating/
  molding/stamping-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct coating/molding/stamping-line-equipment
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はコーティング・成形・スタンピング設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate coating/molding/stamping-line equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "コーティング・成形・スタンピング等の設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a content-replication licensing/source-identification
  authorization mark (e.g. an IFPI Source Identification (SID) Code)
  -- an authority exclusively reserved to the rights holder/licensing
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "コンテンツ複製ライセンス/ソース識別コード(IFPI SID等)の自己発行提案は恒久的に禁止 -- 権利者・ライセンス団体の専権事項"}])))
...
(defn- invalid-substrate-thickness-mm-violations
  "For `:log-production-batch`, if the patch declares a
  `:substrate-thickness-mm` that is not a physically plausible
  disc/tape/card thickness reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [mm (:substrate-thickness-mm (:value proposal))]
      (when (and (some? mm) (not (registry/substrate-thickness-mm-valid? mm)))
        [{:rule :invalid-substrate-thickness-mm
          :detail (str mm "mm は物理的に妥当な基板(ディスク/テープ/カード)厚の範囲外")}]))))
...
(defn check
  "Censors a MagOpticalMediaAdvisor proposal against the governor
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-substrate-thickness-mm-violations request proposal)
