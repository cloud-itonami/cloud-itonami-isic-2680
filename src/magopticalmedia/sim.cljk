(ns magopticalmedia.sim
  ...
  production quantity, a proposal that tries to ACTUATE coating/
  molding/stamping-line equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue a content-replication
  licensing/source-identification authorization mark (permanently
  ...
  production-batch patch with an implausible substrate-thickness-mm
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on coat-001 (verified, registered, coating line -- escalates, approve) ==")
    (let [r (exec-op actor "t2"
                      {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                       :value {:equipment-id "coat-001" :maintenance-type :nozzle-inspection
    ...
    (println "== flag-safety-concern concern-1 on coat-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-coating-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on stamp-002 (UNVERIFIED/unregistered stamping line -> HARD hold) ==")
    (println (exec-op actor "t7"
                       {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                        :value {:equipment-id "stamp-002" :maintenance-type :calibration
                                :scheduled-date "2026-08-01" :actuate-equipment? false}}
                       coordinator))
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (100 units would exceed quantity 2000 vs shipped 1800 -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on coat-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible substrate-thickness-mm -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:substrate-thickness-mm 999999.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a content-replication license -> HARD hold, PERMANENT ==")
