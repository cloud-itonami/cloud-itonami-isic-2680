(ns magopticalmedia.store
  "SSoT for the magnetic/optical media plant-operations coordination
  ...
  `elecequipmfg.store`, `cloud-itonami-isic-2640`'s own
  `consumerelec.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external magnetic/optical-media-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/substrate-thickness-mm/
    ...
    - `equipment`         -- a coating/molding/stamping-line unit's own
    ...
    - `maintenance`       -- a scheduled coating/molding/stamping-line
                             maintenance-window DRAFT against a piece
                             of equipment (`magopticalmedia.registry`'s
    ...
    - `shipments`         -- a proposed outbound magnetic/optical-media
                             product shipment DRAFT
                             (`magopticalmedia.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (magopticalmedia.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (magopticalmedia.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :cd
                :model "CD-R-700MB" :substrate-thickness-mm 1.2
   ...
   "batch-002" {:id "batch-002" :product-type :dvd
                :model "DVD-R-4.7GB" :substrate-thickness-mm 1.2
                :quantity-units 2000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 1800.0
                :last-assessed "2026-07-01"}
   "batch-003" {:id "batch-003" :product-type :magnetic-tape
                :model "LTO-9-18TB" :substrate-thickness-mm 0.04
...
(defn- sample-equipment []
  {"coat-001" {:id "coat-001" :kind :coating-line
               :verified? true :registered? true
               :last-maintenance-date "2026-06-01"}
   "stamp-002" {:id "stamp-002" :kind :data-stamping-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `magopticalmedia.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `magopticalmedia.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coating-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered data-stamping-line unit (blocks any
