(ns magopticalmedia.operation
  "MagOpticalMediaOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MagOpticalMediaAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Magnetic and Optical Media Plant Operations Governor (:govern) and
  ...
  (:require [langgraph.graph :as g]
            ...
            [magopticalmedia.advisor :as advisor]
...
(defn build
  "Compiles a MagOpticalMediaOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `magopticalmedia.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MagOpticalMediaAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Magnetic and Optical Media Plant Operations Governor -- independent censor.
      (g/add-node :govern
